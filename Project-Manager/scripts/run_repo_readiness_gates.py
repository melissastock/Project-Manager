...
GATES: dict[str, tuple[str, Path]] = {
    "readiness": ("production readiness", PM_ROOT / "scripts" / "check_production_readiness.py"),
    "governance": ("downstream governance", PM_ROOT / "scripts" / "validate_downstream_governance.py"),
}
...
def main() -> int:
    ...
    parser.add_argument(
        "--target",
        required=True,
        help="Path relative to the portfolio repository root or absolute (e.g. bg-legal).",
    )
    parser.add_argument(
        "--gates",

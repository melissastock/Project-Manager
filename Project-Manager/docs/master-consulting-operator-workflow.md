...
- **Consulting SKU A / B** (this document): paid offers you sell and deliver — **Clarity Sprint Diagnostic** (A) and **Execution OS Build Sprint** (B). Pilot sequencing for a client repo such as `bg-legal` is in `bg-legal-consulting-pilot-playbook.md` (same directory).
- **Portfolio / PM “checks”** (optional automation in this repo): scripts such as `check_production_readiness.py` and `validate_downstream_governance.py` (under `Project-Manager/scripts/`) validate *documentation and governance artifacts* in a git repo. Those are **not** the consulting SKUs; they do not replace SOW, billing, or delivery for Clarity vs Execution OS.

...
The portfolio uses **`bg-legal/`** as the managed path for the GitHub repo `melissastock/bg-legal` (Case Files **program** working copy). `Project-Manager/config/repos.json` lists this project as **bg-legal**. A Google Drive folder named **Case Files** can remain **intake-only**; it must not be a second git working tree for the same engagement.

## Security (Case Files client material)

- **Raw exports and bulk evidence** stay in **`Archiavellian-Archive`** (or other archive intake) per `Project-Manager/docs/case-files-track-onboarding-2026-04-22.md`. Do not duplicate full evidence trees into `bg-legal/` for convenience.
...
1. **Clone** (from the portfolio repository root):  
...
4. **Remotes**: `python3 Project-Manager/scripts/sync_repo_remotes.py --apply` updates `origin` from `Project-Manager/config/repo-remotes.json` for repos that exist on disk.

## Scripts that use `bg-legal/`

Timeline and docket automation resolve paths via `Project-Manager/scripts/bg_legal_paths.py` (`bg-legal/docs/timeline-evidence/`). Internal portfolio checks:

```bash
python3 Project-Manager/scripts/run_repo_readiness_gates.py --target bg-legal
```

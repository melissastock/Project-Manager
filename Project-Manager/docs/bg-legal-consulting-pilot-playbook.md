...
This playbook pilots **both consulting SKUs** from `master-consulting-operator-workflow.md` (this directory) against the **bg-legal** engagement: first **Clarity Sprint (SKU A)**, then **Execution OS Build (SKU B)**. Follow the master doc for stage gates, payments, and anti-spin rules; this file only adds **repo mapping** and a **pilot checklist**.
...
2. **Engagement pack (optional)** — From the portfolio repository root:  
   `python3 Project-Manager/scripts/scaffold_client_engagement_pack.py --target bg-legal --project-name "bg-legal" --project-focus "legal workflow orchestration and execution OS pilot"`  
...
3. **Delivery scaffold (for PM agile gates)** — If you want this repo to pass internal readiness checks:  
   `python3 Project-Manager/scripts/scaffold_delivery_docs.py --target bg-legal`  
...
If the goal is only to run **portfolio** validators on the `bg-legal` folder (readiness + downstream governance), from the portfolio repository root after the repo exists:

```bash
python3 Project-Manager/scripts/check_production_readiness.py --target bg-legal
python3 Project-Manager/scripts/validate_downstream_governance.py --target bg-legal
```

Or run both checks in one pass (from the portfolio root, with `bg-legal/` present):

```bash
python3 Project-Manager/scripts/run_product_skus.py --target bg-legal
```
